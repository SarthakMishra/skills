...
Read the relevant pattern, then adapt it to the existing accessible primitive and the product's design language.
...
Use the project's established accessible primitive before building overlay mechanics. Verify which contract it supplies: placement, portal, keyboard navigation, Escape, outside interaction, focus restoration, inert background, and scroll management. A generic popover is not automatically a menu or dialog.

- Give an anchored surface a trigger-relative origin. Check placement near viewport edges and after collision flipping.
- Open focus/selection immediately according to the component pattern; do not wait for the visual entrance.
- Support close or reversal during entrance. A closing animation must not later remove a newly reopened instance.
- Keep exiting presentation inert and absent from the tab order. Avoid duplicate active dialogs or ghost click-catching layers.
...
Preserve stable item identity during insertion, removal, and reorder. Keep scroll position and keyboard focus meaningful. If the focused item is removed, choose the next appropriate focus destination deliberately.
...
For async search/filtering, keep input responsive, indicate stale/loading results truthfully, and avoid showing an old response for a new query. A transition cannot repair a request race.
...
Use distance and **recent** directional velocity for dismissal; name units (e.g. CSS px/ms) and tune against the component size. A single average velocity since pointer-down can misread a pause or reversal. Do not copy an unexplained threshold between components. Let an insufficient gesture settle back predictably.
