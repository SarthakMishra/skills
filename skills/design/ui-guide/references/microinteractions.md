...
Bring useful information onto the trigger: a selected filter count, mute state, upload progress, or current setting may eliminate an unnecessary open-and-check. Do not crowd every button with metadata.
...
Use known, appropriate context to improve defaults. Remember an explicit user preference when useful. Do not infer consequential choices or add behavioral tracking merely to make an interaction feel personalized. Surface defaults with meaningful consequences and let the user correct them.
...
Distinguish **received input**, **requested work**, and **confirmed result**. A checkmark must have a defined meaning. Do not signal saved, paid, uploaded, or deleted just because a decorative animation finished.

Prefer existing UI surfaces for feedback: the button, row, input, or affected object. Add a toast when the result would otherwise be missed or belongs outside the current region. Do not emit a toast, banner, icon, live announcement, and modal for the same ordinary success.

Feedback should be timely, understandable, and proportional. Use text or accessible state for essential meaning; motion and color can reinforce it. Keep a persistent status available when a transient flourish ends. Avoid aggressive shaking or celebratory treatment for stressful failures.

For assistive technology, distinguish state semantics (`aria-pressed`, `aria-expanded`, `aria-selected`, etc.) from announcements. Announce meaningful async status once through an appropriate live region; do not narrate every animation frame, every keystroke, or decorative transition.
...
- Delay a busy indicator only to avoid a distracting flash on a genuinely fast operation; give immediate acknowledgment regardless. As a starting point, consider a roughly 150–200ms delay for the indicator, then tune with actual latency. Clear that timer when the operation settles; never postpone useful content or completion to satisfy a minimum animation duration.
...
- For overlapping requests, ensure results correspond to the current input/request. Retargeting an animation is not enough to prevent stale data. Use the project's cancellation/versioning pattern.
...
Evaluate the first, second, and hundredth use. Useful memory can reduce setup; novelty can become friction. Do not silently remove labels or relocate controls merely because a person used them often.
...
## Make ordinary use feel good

Aim for fluency: an obvious trigger, a response tied to the action, stable content, and a clean finish. Reserve a more distinctive moment for something meaningful to the product: completing a creative task, reaching a genuine milestone, or revealing a useful result. Match its intensity to context and repetition.
