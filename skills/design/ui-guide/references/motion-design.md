...
Do not infer frequency from input modality alone. A keyboard-triggered change may still benefit from orientation cues; it must never delay focus, selection, or the next keystroke. Do not animate every row because a list exists.

## Distinguish time variables

**Response latency** is when the UI first acknowledges input. **Delay** is when an animation starts. **Duration** is how long it runs. **Easing** distributes change through that duration. **Spring settling** depends on the simulation and thresholds. **Dwell time** is how long a message remains readable. These are not interchangeable.
...
| ------------------------------------------------- | ------------------------- | -------------------------------- |
| Respond promptly, land gently                     | Ease-out                  | `cubic-bezier(0.16, 1, 0.3, 1)`  |
...
Tune in a curve editor or working preview. If adjusting numbers, compare one variable at a time: shape, then duration, then distance. Keep a small named palette instead of inventing curves component by component.
...
- **Stiffness** controls restoring force: higher generally responds faster.
- **Damping** dissipates motion: higher generally reduces oscillation; excessive damping can feel sluggish.
- **Mass** affects inertia: higher generally slows response at unchanged stiffness/damping.
- **Initial velocity** carries the gesture's direction and speed into release.
- **Rest thresholds** decide when the animation can finish; poorly chosen thresholds produce a long invisible tail.
...
Keep direct manipulation under the pointer. Do not spring-lag the object the person is trying to place precisely. Use elasticity at boundaries and a spring on release when useful. Preserve current position/velocity when retargeting; do not remount the object to restart its entrance.
...
- Let the parent surface establish its presence; content may overlap that motion instead of waiting for a separate stage.
...
- Shared-element transitions must genuinely preserve an object's identity and destination. Crossfade unrelated content rather than pretending it is the same object.
...
| Cheap or bouncy   | Overshoot where precision is expected                      | Critically damped response or a simple tween          |
