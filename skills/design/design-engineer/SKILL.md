...
# Build and verify app interfaces

Own one bounded app-interface task through an integrated implementation and
evidence-backed verification. Automatically use the relevant design companions;
the user invokes this skill once.
...
workflow behind `grill-me`, until the user confirms shared understanding. If
those skills are unavailable, use the fallback interview in the scoping
reference. Install them when the full workflow is needed with
`npx skills add mattpocock/skills --skill grilling grill-me`.
...
ask the user to install the required skill before dependent work. Use
`npx skills add SarthakMishra/skills --skill ux-guide design-system ux-writer react-guide`
for this repository. For an optional companion, continue only with supported
decisions and report the coverage limit. Do not claim a missing skill was used or
silently install it.

For React code, read the `react-guide` skill before authoring or refactoring. It
owns render purity, state, Effects, Actions, data,
composition, refs, transitions, memoization, and server/client boundaries. If it
is unavailable, ask the user to install it before changing React behavior. Use
this skill's references for interface contracts, Tailwind integration, and visual
recipes.

## Workflow

1. Pass the agreed mode, approach, scope, vocabulary, and constraints into one
   shared brief. Reuse accepted answers instead of starting separate interviews.

   Done means the brief names the target, affected consumers, exclusions, and
...
2. Follow [orchestration](references/orchestration.md) to coordinate UX, system
   ...
   Done means the selected journey, component contracts, visual treatment, and
   copy describe the same states and recovery.

3. Implement the chosen treatment using the existing stack, router, data layer,
   and accessible components. Apply reviewed copy in the rendered UI. For React,
   apply the relevant `react-guide` rules
   and keep feature data and business rules in their existing owners.

   Done means the requested interface works in its real consumer, not only in an
   isolated component or design document.
...
4. Verify the journey, system, visual states, copy, and runtime behavior together.
   ...
   Done means the handoff separates verified checks from unavailable or proposed
   checks and names any remaining material limit.
...
| `react-guide`                                                      | React render, state, Effects, Actions, data, composition, refs, and boundaries. |
| [React and Tailwind](references/react-tailwind.md)                 | Interface-specific Tailwind integration and controlled view recipes.            |
